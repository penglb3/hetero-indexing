...
sketch* sketch_construct(uint32_t width, uint32_t depth, uint32_t seed, int mode){
    ...
    if(mode == BLOOM_FILTER && width >= sizeof(*sk->counts)*8 ){
        ...
        sk->counts = calloc(depth * width * sizeof(__typeof__(*sk->counts)) / 64, 64);
...
int countmin_inc_explicit(sketch* cm, const void* data, uint32_t len, void* ext_hash){
    ...
    for(int i=0, i_c; i<cm->depth; i++){
        idx = i * cm->width + (hash[0] & (cm->width - 1));
        cm->counts[idx]++;
        #if defined(COUNTMIN_CD_FACTOR) && !defined(COUNTMIN_CD_INTERVAL)
        i_c = idx&(64 / sizeof(*cm->counts) - 1);
        for(int j=i_c; j < i_c + 64 / sizeof(*cm->counts); j++){
            cm->counts[j] *= COUNTMIN_CD_FACTOR;
        }
        #endif
